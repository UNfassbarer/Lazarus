...
begin
  ...
  begin
    if a = 'area' then
      E_area.Text := FloatToStrF(val1 * val1, ffFixed, 3, 7)
    else
      E_scope.Text := FloatToStrF(4 * val1, ffFixed, 3, 7);
  end;
end;

procedure TForm1.CB_areaChange(Sender: TObject);
begin
  if CB_area.Checked then Calculate('area') else E_area.Clear;
end;

procedure TForm1.CB_scopeChange(Sender: TObject);
begin
  if CB_scope.Checked then Calculate('scope') else E_scope.Clear;
end;
...
procedure TForm1.RB_KreisChange(Sender: TObject);
begin
  L_1.Caption := 'Radius r';
  L_2.Caption := 'not necessary';
  Check_Box_Input();
end;

procedure TForm1.RB_quardatChange(Sender: TObject);
begin
  L_1.Caption := 'Length a';
  L_2.Caption := 'not necessary';
  Check_Box_Input();
end;

procedure TForm1.RB_rechteckChange(Sender: TObject);
begin
  L_1.Caption := 'Length a';
  L_2.Caption := 'Length b';
  Check_Box_Input();
end;
